...
| Workspace and CI foundation | Complete | pnpm/Turbo/strict TypeScript/Biome workspace, frozen lockfile, Apache-2.0 project files, full-SHA-pinned actions, and historical green public CI run [`29805311450`](https://github.com/reachjalil/liteMCP/actions/runs/29805311450) | The public run predates this working tree; merge-time CI, tagged release, artifact signing, dependency review, and security scans remain |
...
| Identity and Better Auth | Partial | Signup kill switch, Resend delivery seam for verification/reset/invitations, signup-aware first-organization UI, idempotent tenant bootstrap, role entities/assignments, bounded group/claim ingestion, mapping evaluation, and epoch invalidation are implemented in code; IdP control records encrypt client secrets with a tenant-derived AES-GCM key | Checked-in cloud signup stays disabled; no live email, Entra/Okta/SAML/SCIM proof; IdP records are not bridged to Better Auth provider registration; Better Auth membership/ban/SCIM events do not automatically call deprovision/epoch logic |
