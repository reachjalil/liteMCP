...
| Workspace and CI foundation | Complete | pnpm/Turbo/strict TypeScript/Biome workspace, frozen lockfile, Apache-2.0 project files, full-SHA-pinned actions, and historical green public CI run [`29805311450`](https://github.com/reachjalil/liteMCP/actions/runs/29805311450). The working tree adds Harness apply/convergence, Python SDK, generated-schema/upgrade parity, production audit, structural workflow policy, local D1, dependency-review, secret-scan, render/build, exact-digest container scan/evidence, digest-only publication, and required-aggregator gates | The public run predates these gates; their first merge-time run, branch required-check rule, tagged release, signing, and external release acceptance remain |
...
| Public Astro site | Partial | Site code includes signup-aware login/recovery/invitation flows plus privacy and terms pages, alongside the responsive site and console | Current pages are not deployment proof; formal accessibility/performance/privacy review remains |
...
| MCP gateway | Partial | Authenticated initialize/notifications, `tools/list`, and `tools/call`; builtin and remote HTTP composition; aliases, policy/schema/approval/quota checks, bounded execution, and audit fail-closed behavior remain. The gateway now records self-reported initialize attribution separately, emits one aggregate discovery and one terminal tool-attempt fact, preserves denial/rule/audit context, and measures total plus per-fetch upstream latency/bytes across retries through a fail-open recorder | External conformance, resources/prompts/resume, genuine SSE/streaming, authenticated upstream discovery, scheduled probes, managed-cloud DNS pinning, and named-client proof |
...
| CLI and SDKs | Partial | Operational CLI, TypeScript SDK, and dependency-free Python SDK cover the implemented management lifecycle with focused request-shape tests; the runnable local-composition example remains | Published-package smoke, complete compatibility/resource coverage, production auth/context flows, stable compatibility policy, and authoring SDK |
...
| Managed cloud deployment | Partial; earlier public preview live | Worker version `519323e3-dbfb-418a-92a4-9aeb16afe973` remains only the earlier site/health/session smoke. The working tree adds a staging target, exact-CI-built target archives with SHA-256 manifests, Worker UUID/tag and migration evidence, read-only remote Durable Object lifecycle gating, explicit production promotion, mandatory authenticated read-only MCP smoke, tenant authority, D1 rate limiting, email/Sentry gates, tenant-indexed Analytics Engine emission, and a sibling capped exact analytics-feed/query path | None of the working-tree changes is claimed deployed. Staging resources and its first Worker are absent; production has not applied the final checked-in Durable Object migration; the separate lifecycle operation and routine workflow are therefore fail-closed. The current GitHub repository has no branch rule or environment protection rules; no privileged/analytics/OAuth/client, rollback, load/security, or SLO acceptance exists, and the current analytics API does not query Analytics Engine history |
