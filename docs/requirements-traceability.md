...
| Managed cloud deployment | Blocked for live acceptance | `apps/managed-cloud`, Worker assets/API, D1/KV bindings, Cloudflare adapter | Type/test/build; local D1 migration; Wrangler dry-run | Authenticated resource bootstrap, stronger mutation authority, custom domain, full deployed smoke and rollback |
...
| Public site and console | Preview | Astro site, React API-backed console, product/status/security content | Astro check/build and local desktop/mobile browser QA | Public deploy, formal accessibility/performance audit, full CRUD journeys, live auth and operational surfaces |
